...
"""
    ...
    def _build_describe_state_user_prompt(self, description: str) -> str:
        """Формирует пользовательский промпт для «Описать состояние»."""
        return f"""
Пользователь описывает своё состояние:

{description}

Ответь естественно, как в живом разговоре. Будь поддерживающим, бережным и внимательным.
Задай уточняющие вопросы, если нужно. Не используй шаблоны.
"""
    ...
    # ==================== РЕЖИМ 4: «ОПИСАТЬ СОСТОЯНИЕ» (ЖИВОЙ ДИАЛОГ) ====================
