...
# ==================== НОВЫЙ ОБРАБОТЧИК: ОТКРЫТИЕ ДНЕВНИКА ИЗ НАПОМИНАНИЯ ====================
...
async def reminder_open_diary(callback: CallbackQuery, state: FSMContext, db_session: AsyncSession):
    ...
    class FakeMessage:
        def __init__(self, user_id, from_user, chat):
            self.from_user = from_user
            self.chat = chat
            self.text = "➕ Новая запись"
        
        async def answer(self, *args, **kwargs):
            # Просто заглушка, чтобы не сломать start_new_diary_entry
            pass
    
    fake_from_user = type('obj', (object,), {'id': callback.from_user.id})
    fake_chat = type('obj', (object,), {'id': callback.from_user.id})
    fake_message = FakeMessage(callback.from_user.id, fake_from_user, fake_chat)
