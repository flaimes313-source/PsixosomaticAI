...
async def _start_describe_state_flow(
    ...
    dialog_message = await message.answer(
        ...
        "Сома сама начнёт диалог и задаст несколько простых вопросов, чтобы помочь тебе разобраться.\n\n"
        "Я буду уточнять только то, что действительно важно для понимания твоего состояния, и вместе мы посмотрим, что могло на него повлиять.\n\n"
            ...
            if dialog_message_id:
                try:
                    ...
                    try:
                        ...
                        logger.warning(f"delete_message also failed: {del_e}")
...
async def describe_back_to_menu(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        logger.warning(f"Failed to delete dialog message: {e}")
